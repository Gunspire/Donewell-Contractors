...
import { COMPANY, QUOTE_PREVIEW_STORAGE_KEY } from "../config/company";

type SurfaceType = "block_paving" | "porcelain" | "sandstone" | "tarmac" | "gravel" | "concrete";
type ServiceType =
  | "driveway_install"
  | "patio_install"
  | "driveway_extension"
...
type QuotePreviewPayload = {
  ...
  job: { surfaceTypeLabel: string; serviceLabel: string; areaSqm: number };
  items: LineItem[];
...
const PRICING = {
  ...
  service: {
    driveway_install: { basePence: 900_00, perSqmPence: 85_00 },
    patio_install: { basePence: 650_00, perSqmPence: 65_00 },
    driveway_extension: { basePence: 500_00, perSqmPence: 55_00 },
    edging_kerbs: { basePence: 280_00, perSqmPence: 12_00 },
    brickwork: { basePence: 420_00, perSqmPence: 20_00 },
    drainage_groundworks: { basePence: 380_00, perSqmPence: 18_00 },
  } satisfies Record<ServiceType, { basePence: number; perSqmPence: number }>,
  addons: {
    skip: { label: "Waste removal / skip", pence: 260_00 },
    excavation: { label: "Excavation / dig-out (if required)", pence: 480_00 },
    edging: { label: "Edging / kerbs (if required)", pence: 220_00 },
    drainage: { label: "Drainage channel / soakaway (if required)", pence: 390_00 },
    sealing: { label: "Sealing (optional, after curing)", pence: 160_00 },
  },
} as const;

const SURFACE_TYPE_LABEL: Record<SurfaceType, string> = {
  block_paving: "Block paving",
  porcelain: "Porcelain paving",
  sandstone: "Sandstone paving",
  tarmac: "Tarmac",
  gravel: "Gravel",
  concrete: "Concrete",
};
...
const SERVICE_LABEL: Record<ServiceType, string> = {
  driveway_install: "Driveway install",
  patio_install: "Patio install",
  driveway_extension: "Driveway extension",
  edging_kerbs: "Edging / kerbs",
  brickwork: "Brickwork / garden walls",
  drainage_groundworks: "Drainage / groundworks",
};
...
export function InvoiceQuoteSection() {
  ...
  const [surfaceType, setSurfaceType] = React.useState<SurfaceType>("block_paving");
  const [service, setService] = React.useState<ServiceType>("driveway_install");
  ...
  return (
    <section className="bg-slate-50 py-12 lg:py-16 border-t border-slate-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading subtitle="Quote Builder">Generate an estimate summary PDF</SectionHeading>
            ...
            <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 sm:p-6">
              ...
              <div className="mt-5 space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Anything we should know (access, parking, levels, drainage, etc.)"
                />
              </div>
            </div>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const payload: QuotePreviewPayload = {
                        ...
                        job: {
                          surfaceTypeLabel: quote.surfaceTypeLabel,
